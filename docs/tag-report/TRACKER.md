...
**Status:** Design complete and fully reconciled through TWO review gates (2026-07-01: initial security/QA round, then a final consistency sweep that caught and fixed several bugs the first round's fixes had introduced or missed — see §4a/§4b/§4c for the full resolution log). **Implementation started 2026-07-02** on the `tag-report` branch/worktree. A third, customer-journey-driven verification pass on 2026-07-03 found and fixed 6 UI gaps, then a deeper end-to-end functional audit the same day found and fixed 6 more — see "Final end-to-end verification pass" below for the second round. Remaining pre-implementation open item is a business decision, not an engineering gap: DESIGN.md §7 item 0 (citation-erasure policy).
...
## Follow-on: Crystal `tag-analyst` conversational skill (2026-07-03)

Not part of Tag Report's original DESIGN.md/TRACKER.md scope — a follow-on feature request so Crystal (Xperiq's AI copilot) can answer conversational questions about tags/tag groups (metadata, Tag Reports, trails, cross-survey topics, coverage), with automatic context-scoping when a user is viewing a Tag Report page. Built by 3 parallel agents (CrystalOS, Backend, Frontend), same team-dispatch pattern as the core feature.

| Area | Status | Notes |
|---|---|---|
| CrystalOS: tools + skill + scope wiring | ✅ Done | 5 new read-only, org-scoped tools (`list_tags`, `get_tag_report`, `get_tag_report_trail`, `propose_view_tag_report`, `propose_generate_tag_report`) in `crystal/registry.py`/`crystal/tools.py`; new `tag-analyst` skill (`crystalos/skills/tag-analyst/`) with explicit trust-layer-fidelity prompting; `scope='tag'` wired end-to-end from `main.py` through `CrystalContext.tag_ids` to semantic routing (`_routing_hint_for_scope`) and tool prefetch. `get_tag_report`'s shaping logic (`_build_tag_metric_tracks`/`_build_tag_disclosure`) is a faithful line-for-line port of `backend/src/lib/tagReportView.ts` so Crystal's prose and the Tag Report page never disagree. Zero write capability — verified by a dedicated test grepping executor source for INSERT/UPDATE/DELETE. 1811/1811 crystalos tests passing (26 new). |
| Backend: `scope='tag'` on Crystal chat routes | ✅ Done | `POST /:scope/crystal/stream` scope validation extended to `['survey','org','tag']`; `loadCrystalContext` gets a new tag branch that validates the tag belongs to `req.orgId` before loading anything (falls through to org-wide behavior on a cross-org tag_id — no existence leak); credit metering unchanged, no bypass added for the new scope. Purely additive — survey/org behavior confirmed byte-for-byte unchanged. 1274/1275 backend tests passing (11 new). |
| Frontend: tag-scoped Crystal context | ✅ Done | New `CrystalCtx.focused_tag_id`/`focused_tag_name` fields (additive, `SurveyScope` type untouched); `streamScope` derivation extended to a third `'tag'` case; `TagReportPage`/`TagReportTrailPage` auto-set tag context on mount (cleared on unmount) — `TagReportsIndexPage` deliberately excluded (lists many tags, no single focus); new `view_tag_report`/`generate_tag_report` action-proposal dispatch cases wired into `executeAction`. **Found and fixed a real pre-existing bug**: `TagReportPage` was calling `setScope(tagId)` — stuffing a tag ID into the survey-scope slot, which silently broke `isAll` on that page and would have hidden the new tag-context UI. Removed in favor of the new `setCrystalCtx` wiring. 1071/1071 app tests passing (11 new). |
| Integration + regression pass | ✅ Done | Traced the full three-way contract by reading the actual code end-to-end (not just trusting agent self-reports): `main.py` correctly forwards `tag_id`/`tag_ids` into `CrystalInput`; `_build_ctx` correctly prioritizes explicit `tag_ids` over the singular `tag_id` backend sends; all 5 new tool schemas match their prefetch call sites' param names exactly; proposal names match end-to-end (`propose_view_tag_report`/`propose_generate_tag_report` → identity-aliased → `view_tag_report`/`generate_tag_report` → matched exactly by frontend's switch cases); `_load_org_tag` correctly gates `get_tag_report`/`get_tag_report_trail` before any other query, treating "wrong org" identically to "not found" (no existence leak) — same pattern as the original Response Detail fix. **One real, if minor, contract deviation found and fixed**: `get_tag_report`'s `survey_breakdown[]` emitted a raw `response_count` field where the backend's authoritative shape (`tagReportView.ts`) emits `trust_score` (an `approximateTrustScore()` display/sort proxy) — cosmetic only (sort order was already correct since the proxy is monotonic in response count), but fixed for field-name fidelity since "faithful port" was the explicit design intent. Final: backend 1274/1275, app 1071/1071, crystalos 1812/1812 (1 new regression test for the fix). Zero real regressions. |
| Security review | ✅ Done | Fresh adversarial pass against the actual shipped code. Two real findings, both fixed and regression-tested, neither an exploitable cross-tenant leak today (both contained by redundant downstream org-checks) but real fail-closed gaps worth closing rather than relying on redundancy: (1) `POST /:scope/crystal/stream` built the `scope`/`tag_id` it forwarded to CrystalOS from the raw route param + raw client body instead of `loadCrystalContext`'s org-validated `ctx.scope`/`ctx.tag_id` — a cross-org `tag_id` still got labeled `scope='tag'` downstream with the unvalidated id attached, relying entirely on every CrystalOS tool independently re-checking ownership rather than failing closed at this layer. Fixed by deriving `agentBody`'s scope/tag fields from `ctx.scope`/`ctx.tag_id` post-validation, falling back to `'org'`/no-tag-context on any validation failure or context-load error. An existing test had explicitly asserted the vulnerable behavior as correct in its own comment — rewritten to assert the fail-closed behavior instead, plus a new test confirming the happy path still forwards real validated tag data. (2) `propose_view_tag_report`/`propose_generate_tag_report` didn't call the shared `_load_org_tag` gate that `get_tag_report`/`get_tag_report_trail` use, inconsistent with the module's own stated invariant — no live exploit (both are pure proposal-builders with zero DB/HTTP calls; the actual mutation/navigation only happens after user-confirmed `executeAction`, independently org-scoped downstream) but fixed for defense-in-depth and as a bonus now surfaces the tag's real validated name in the proposal title instead of a generic one. `crystalHandler` (the non-streaming fallback) was independently confirmed to have been built correctly from the start — it already derived everything from `ctx.scope`/`ctx.tag_id`, isolating the gap to the one streaming route. Everything else (org-scoping origin tracing from Clerk JWT through to `ctx.org_id`, existence-leakage parity, the zero-fresh-generation invariant through the new proposal path, rate limiting/credit metering with no bypass, `list_tags`' parameterized ILIKE, reused group-tool scoping under the new caller, frontend's `proposal.params.tag_id` never being trusted for anything sensitive without server-side re-validation) traced end-to-end and confirmed clean. Final: backend 1275/1276 (1 pre-existing unrelated RED), app 1070/1071 (1 confirmed pre-existing flaky test, passes 20/20 in isolation, unrelated), crystalos 1814/1814 (2 new regression tests). Zero real regressions. |
...
## Customer-journey UI review + fix pass (2026-07-03)

Requested walkthrough of the full customer journey (every entry point, happy path, error states, Trail page) surfaced 6 real gaps in the shipped frontend — none were backend/CrystalOS bugs; all were frontend polish/correctness issues that would have reached real users. All 6 fixed and regression-tested same day.

| # | Gap | Fix | Files |
|---|---|---|---|
| 1 | **Generic error masking.** `TagReportNewPage` always showed a fixed "Could not start the report. Please try again." regardless of whether the real reason was zero-surveys (400), rate-limit (429), or tag-not-found (404) — the specific backend message was computed but discarded. | Changed `useTagReport`'s `generate()` to return `{runId, error, inFlightNotice}` directly instead of just `string \| null` — avoids a stale-closure bug where reading hook state right after an `await` doesn't see the value that same call just set. Page now shows the real message, falling back to the generic string only when none exists. | `hooks/useTagReport.ts`, `pages/experience/TagReportNewPage.tsx` |
| 2 | **`InFlightRunBanner` unreachable in the real flow.** Fully built and unit-tested in isolation, but `TagReportNewPage` navigates away immediately on success (discarding its hook instance's `inFlightNotice` state) and `TagReportPage` mounts a fresh hook instance that never calls `generate()` — so the banner could never actually render for a real user. | Forward the notice through React Router navigation `state` from the page/component that calls `generate()` to the page that renders the banner. Also discovered and fixed a **better underlying signal while at it**: the old code re-derived an "is this in-flight?" guess via a timestamp-age heuristic from a second API call; the backend's `attached_to_existing` field (from `insertGroupInsightRunWithConcurrencyGuard`'s real DB-level 23505-conflict check) was already being returned and silently discarded — now used directly, no heuristic. | `hooks/useTagReport.ts`, `pages/experience/TagReportNewPage.tsx`, `pages/experience/TagReportPage.tsx`, `components/CrystalPanel.tsx` (same fix applied to the Crystal-triggered generation path), `types/tagReport.ts` |
| 3 | **Report page title said "— Group Report".** Borrowed `groups.groupReportTitle` from the older, unrelated Group Insights feature — the page/tab title never said "Tag Report" anywhere, on both the browser tab and the on-page H1. | New `tagReport.page.title` locale key (`"{name} — Tag Report"`), used in both `useSetPageTitle` and the `PageHeader`'s `title` prop. | `pages/experience/TagReportPage.tsx`, `locales/en.ts` |
| 4 | **Raw snake_case warning identifiers leaked to the UI.** `MetricHeadlineCard`'s comparability-warning pills rendered `w.warning_type` verbatim — a customer would see a chip literally reading `cadence_mismatch` — the only unlocalized strings in an otherwise fully-localized feature. | Added `tagReport.metricCard.warningType.*` locale keys (one real sentence per known type: `temporal_offset`, `staleness`, `question_type_mismatch`, `scale_mismatch`, `cadence_mismatch`) plus an `unknown` fallback for any future/unrecognized value — never renders the raw identifier. | `components/tag-report/MetricHeadlineCard.tsx`, `locales/en.ts` |
| 5 | **Duplicate "Generate Report" button routed to the wrong (paid) feature.** `SurveysListPage`'s toolbar button, shown whenever ≥1 tag was filtered, always called the older Group Insights flow (real fresh-LLM generation, `GROUP_REPORT` route) — presenting near-identical copy/icon to Tag Report's own entry points on the same page. A customer filtering to a single tag and clicking it got silently charged for fresh AI generation instead of the free rollup they likely meant. | Single-tag selection (the common case, and the only case Tag Report actually supports — it's always scoped to one tag) now routes to Tag Report's mode-picker instead. Multi-tag selection — a genuinely different capability Tag Report cannot replicate — keeps the legacy flow unchanged, just relabeled ("Generate Group Insights", distinct icon, a tooltip stating it uses credits) so its identity and cost are never confused with the free path. Also fixed a smaller, purely cosmetic instance: `TagsSettingsPage`'s per-tag hover hint already correctly navigated to Tag Report but displayed the same stale "Group Report" label. | `pages/SurveysListPage.tsx`, `pages/settings/TagsSettingsPage.tsx`, `locales/en.ts` |
| 6 | **"Automated schedules active" stat permanently stuck at 0.** `TagReportsIndexPage` computed the stat from `reports[].automated_enabled`, but the backend's index endpoint never selected that field at all — always `undefined`. | Added `COALESCE((t.program_config -> 'tag_report_automated' ->> 'enabled')::boolean, FALSE) AS automated_enabled` to the index query, reading the same `program_config` JSONB path the Automated-mode due-tags sweep (`tagReportScheduler.ts`) already treats as the source of truth. | `routes/survey-groups.ts` |

**Test impact:** 39 new/updated tests across `useTagReport.test.ts`, `TagReportNewPage.test.tsx`, `TagReportPage.test.tsx`, `MetricHeadlineCard.test.tsx`, `SurveysListPage.test.tsx`, `CrystalPanel.test.tsx`, `tagReport.test.js` (backend). Final: backend 1277/1278 (1 pre-existing unrelated RED), app 1082/1082, crystalos 1814/1814 (untouched by this pass). Zero regressions.

## Final end-to-end verification pass (2026-07-03)

Requested a full functional/trust-layer/integration audit ("make sure customer will get all the value they need") on top of the customer-journey UI pass above. Dispatched 3 independent, adversarial verification agents (Manual+Custom Range end-to-end trace; Trust Layer + Trail audit; entry points + Crystal `tag-analyst` chain), each tracing real code rather than trusting prior docs. Two came back clean (all entry points, the full Crystal conversational chain, Manual mode, R-T2/R-T2a/R-T3/R-T5 all confirmed working end-to-end against the actual current code). The other two surfaced 6 real gaps — including one severe, previously-undetected crash bug — all fixed and regression-tested same day.

**Also resolved as part of this pass: Automated mode has no customer-facing enable/configure UI, by design, not a bug.** Verified directly against `DESIGN.md` §6: Automated mode is explicitly Phase 3, gated on the Automation Hub's workflow scheduler being production-ready — "not something Tag Report can unblock on its own." The `automated_enabled` stat fixed in the pass above is correctly wired but will legitimately read 0 for every org until that cross-team dependency lands; this is expected, not a regression.

| # | Gap | Severity | Fix | Files |
|---|---|---|---|---|
| 1 | **Custom Range's same-checkpoint survey silently vanished instead of showing "no comparison available."** R-C1's post-QA AC (DESIGN.md line 137) explicitly requires a survey whose baseline/latest resolve to the same checkpoint to show a flat, disclosed snapshot — never a fabricated 0%, and never silent omission. CrystalOS correctly excluded it from `eligible_survey_ids` (it can't vote on a trend) but that also meant it never reached `group_insights.survey_ids` at publish time, so `ComparisonWaveCard`'s per-survey breakdown just didn't show it at all — while it still appeared in the Disclosure Banner's Included list with a response count, misleadingly implying it contributed to the comparison. Zero test coverage of the real `buildMetricTracks`/publish path let this slip through both the 2026-07-01 QA round and the customer-journey pass above. | High — a real trust-layer violation of an explicit, hard-won AC | Added a `no_comparison_survey_ids` list, tracked separately per metric in `node_apply_trend_eligibility_gate` and persisted into `metric_json` (deliberately kept OUT of `eligible_survey_ids`/`survey_ids`, which must stay strictly trend-eligible for citations + the "N trend-eligible surveys" count). `tagReportView.ts` unions it into `survey_breakdown` for Custom Range only — the existing "no delta event → `no_comparison_available: true`" logic already handled the rest correctly once the survey was actually present. | `crystalos/graphs/tag_report.py`, `backend/src/lib/tagReportView.ts` |
| 2 | **Trail page crashed on every real visit.** `TagReportTrailResponse` declared `{tag_id, tag_name, runs, sources}`; the real `GET .../tag-report/:runId/trail` endpoint has only ever returned `{run_id, lineage, sources, truncated}` — scoped to one run's provenance, not a tag's history. `data.runs` was therefore always `undefined`, and `TagReportTrailPage`'s `runs.map(...)` threw immediately. This is not new — it appears to have shipped this way from initial implementation and was missed by every prior review pass (including the 2026-07-02 integration reconciliation, which caught 8 other contract mismatches but not this one, and the 2026-07-01 QA round). | **Severe — a full page crash, zero customer value delivered on the Audit Trail page** | `api.ts`'s own code comment already said `getTagReportHistory` (paginated, all 3 modes) was the intended source for the Trail page's run list — it just wasn't being used for that. Rewrote `useTagReportTrail(tagId, runId)` to source `runs` from `getTagReportHistory` and `sources`/`truncated` from `getTagReportTrail`; `TagReportTrailResponse` corrected to match the real endpoint; `tagName` now fetched via `api.getTagSurveys` in the page itself (mirroring `TagReportPage.tsx`'s identical pattern), since neither trail endpoint has ever returned it. | `app/src/types/tagReport.ts`, `app/src/hooks/useTagReport.ts`, `app/src/pages/experience/TagReportTrailPage.tsx` |
| 3 | **`tag-report-history`'s row shape didn't match its own consumer's type either.** Even after fixing #2 to point at the right endpoint, that endpoint returned `id` (not `run_id`) and never computed `metric_tracks_narrated` at all — the frontend's `TagReportTrailEntry` type requires both. | High (compounds #2 — the "right" endpoint was itself broken) | Aliased `r.id AS run_id`, added a `LEFT JOIN LATERAL` counting real `group_insights` rows per run for `metric_tracks_narrated` — a genuine per-run aggregate, not a placeholder. | `backend/src/routes/tags.ts` |
| 4 | **Confusing "Back → 3" run-history link copy**, flagged as a smaller item in the original customer-journey gap list but not included in that pass's fix set. Once `metric_tracks_narrated` was fixed (#3) to be a real number, the copy itself was still nonsensical — "Back" on a forward-navigation link to view that run. | Low (cosmetic, but customer-visible on every row) | New `tagReport.trailPage.viewRun` locale key ("View report ({count} metrics)") replaces the `common.back` + bare number. | `pages/experience/TagReportTrailPage.tsx`, `locales/en.ts` |
| 5 | **Silent truncation of the parent_run_id lineage walk.** The backend's `/trail` endpoint computes `truncated: hops >= 10 && currentId !== null` (bounded lineage-walk cap), but this field was never read anywhere in the frontend — a tag with a very long causal chain would truncate with zero indication. | Medium | `truncated` now flows through `useTagReportTrail` and renders a note above the Sources section when true. | `hooks/useTagReport.ts`, `pages/experience/TagReportTrailPage.tsx`, `locales/en.ts` |
| 6 | **R-T1 (statistical floor) wasn't visually distinguished in the Disclosure Banner's Included list.** `TagReportRunSource.trend_eligible` was available on the data but the Included list rendered every survey identically regardless — the Trail page's Sources list already made this distinction (reusing an imprecise "Insufficient agreement" label, left as-is — out of scope for this fix), but the primary report page's own disclosure banner was the one place a below-floor survey looked no different from a trend-eligible one. | Medium | Added a `belowStatFloor` badge (matching the excluded list's existing amber-chip visual language) shown only when `!s.trend_eligible`. | `components/tag-report/DisclosureBanner.tsx`, `locales/en.ts` |

**Confirmed clean, no changes needed (verified against live code, not prior docs):** all Survey List / Tags Settings / Experience Hub / Reports Index entry points; the full Crystal `tag-analyst` conversational chain end-to-end including both action proposals and the fail-closed scope-degradation path; Manual mode's zero-fresh-generation invariant and top-N/backfill resolution; R-C2's blended-zone temporal-offset formula; R-T2/R-T2a's agreement-floor and single-survey-naming logic (both the trivial and general case); R-T5's full citation drill-down chain including the legacy-citation non-clickable fallback; DataBus invalidation; the daily rate limit on both Manual and Custom Range triggers.

**Test impact:** 3 new CrystalOS tests (`test_tag_report.py`), 2 new backend tests (`tagReport.test.js`, `tags.test.js`) + 1 rewritten test file (`tags.test.js`'s history describe block), 1 new frontend component test (`DisclosureBanner.test.tsx`) + 2 rewritten hook/page test files (`useTagReport.test.ts`, `TagReportTrailPage.test.tsx`). Final: backend 1279/1280 (1 pre-existing unrelated RED), app 1087/1087, crystalos 1817/1817. Zero regressions.
