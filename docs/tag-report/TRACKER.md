...
**Status:** Design complete and fully reconciled through TWO review gates (2026-07-01: initial security/QA round, then a final consistency sweep that caught and fixed several bugs the first round's fixes had introduced or missed — see §4a/§4b/§4c for the full resolution log). No implementation started — this tracker sequences the work. Remaining open item is a business decision, not an engineering gap: DESIGN.md §7 item 0 (citation-erasure policy).
...
- **Recency-ranking query**: LATERAL join per survey → most recent `insight_checkpoints_v2` row, filtered `status = 'ready'` (this filter is what makes "zero fresh generation" enforceable at the query level — a survey with only a generating/failed checkpoint is invisible and correctly excluded). Over-fetch candidates (e.g. `effective_max_surveys * 3`) so the backfill loop has room without a second round-trip in the common case. **Corrected post-final-review (was pointed at the wrong table — `insight_reports` instead of the confirmed-live `insight_checkpoints_v2`):** new partial index needed: `CREATE INDEX idx_insight_checkpoints_v2_survey_window ON insight_checkpoints_v2 (survey_id, org_id, window_end DESC) WHERE status = 'ready'` — verify the exact column names against `insight_checkpoints_v2`'s actual schema (`20240523000000_insight_checkpoints_v2.sql`) before writing this, since it was designed against the wrong table originally and column names may differ slightly (e.g. checkpoint timestamp column naming) from what's assumed here.
...
- **Zero fresh AI generation is enforceable at the query layer**, not just convention — the LATERAL subquery's `status = 'ready'` filter and the complete absence of any `agentsClient`/CrystalOS call in the selection path means there is no code path in this feature that can trigger generation. Worth stating explicitly in the PR description for Riley's review.
