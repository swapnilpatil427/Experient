...
async def _run_skill_stream(
    ...
    try:
        ...
        # hard force is safe for each. Falls through to normal semantic
        # routing if the skill is somehow unregistered, rather than
        # dead-ending the request. Every caller that matches neither condition
        # (inp.surface == "insights" AND no taxonomy-question match — the
        # overwhelming majority, unchanged since before Wave 15) is untouched.
